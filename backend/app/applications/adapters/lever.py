"""Lever ATS adapter (Task 3, slice 3c).

Covers `jobs.lever.co/<company>/<posting-id>/apply` postings. Lever's
public application form is plain HTML (labeled inputs, `urls[...]`-style
bracketed names for link fields, a resume file input, and EEO
radio/checkbox fieldsets) so the shared schema extractor handles it
without special-casing; no live posting was available to verify markup, so
this stays conservative and standard rather than guessing at undocumented
structure.
"""
...
class LeverAdapter:
    ...
    async def detect(self, url: str, page) -> bool:
        ...
        if host == LEVER_HOST:
            return True
        try:
            return bool(await page.locator('form[action*="lever.co"]').count())
        except Exception:
            return False

    async def open_application(self, page, job_url: str) -> None:
        await page.goto(job_url, wait_until="domcontentloaded", timeout=30000)
        # Lever posting pages usually render the application form inline at
        # the bottom of the page, but some postings gate it behind an
        # "Apply for this job" link/button. Click if present, otherwise
        # assume the form is already visible.
        ...
        if await apply_control.count():
            try:
                ...
                logger.debug("Lever apply-button click was not needed or failed")

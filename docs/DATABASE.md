...
| `clerk_user_id` | `uuid` | NOT NULL, UNIQUE, FK → `auth.users.id` | Links to Supabase Auth |
...
**RLS:** `USING (clerk_user_id = auth.uid())`
...
**RLS:** `USING (user_id = (SELECT id FROM users WHERE clerk_user_id = auth.uid()))`
...
| Table | RLS Predicate |
|---|---|
| `users` | `clerk_user_id = auth.uid()` |
| `user_model_settings` | Via `users.clerk_user_id = auth.uid()` join |
| `user_preferences` | Via `users.clerk_user_id = auth.uid()` join |
| `documents` | Via `users.clerk_user_id = auth.uid()` join |
| `resume_personas` | Via `users.clerk_user_id = auth.uid()` join |
| `applications` | Via `users.clerk_user_id = auth.uid()` join |
| `agent_runs` | Via `users.clerk_user_id = auth.uid()` join |
| `cover_letter_versions` | Via `users.clerk_user_id = auth.uid()` join |
| `interview_sessions` | Via `users.clerk_user_id = auth.uid()` join |
| `salary_reports` | Via `users.clerk_user_id = auth.uid()` join |
| `leads` | Via `users.clerk_user_id = auth.uid()` join |
| `linkedin_outreach_queue` | Via `users.clerk_user_id = auth.uid()` join |
| `ats_scores` | Via `users.clerk_user_id = auth.uid()` join |
...
| 0018 | `fix_rls_clerk_user_id.sql` | RLS predicate fix for clerk_user_id column |
